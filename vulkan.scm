...
(library (vulkan)
  ...
  (define (make-application-info app-name)
    (let ((app-info (make-foreign-object vk-application-info))
      ...
      (write-cstring app-info
		     vk-application-info
		     application-name
		     app-name)

      (ftype-set! vk-application-info
		  (application-version) app-info vk-version)

      (write-cstring app-info vk-application-info engine-name "kaladin")
